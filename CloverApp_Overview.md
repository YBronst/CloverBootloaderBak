...
**Status**: Legacy / Maintenance mode. The original author is inactive, and the codebase relies on techniques that are no longer supported by Apple.

**What to do now?**
1.  **For Users**: If you are on macOS 11 (Big Sur) or newer, most of Clover.app's features (like RootRW or automated RC script installation) will not work correctly. It is recommended to use the app only for simple EFI mounting or to check the Clover revision.
2.  **For Developers**:
    *   To fix the app, the `CFRunLoopRun()` call in `AppDelegate` must be removed/refactored.
    *   `NSWindowFix.m` should be deleted.
    *   The installer should be rewritten to use `SMJobBless` or `ServiceManagement` framework instead of manual `cp` to `/Library/`.
3.  **Removal**: If the app "refuses to open," it is best to remove it and its associated daemon (`/Library/Application Support/Clover/CloverDaemonNew`) to avoid system logs being flooded with crash reports.
